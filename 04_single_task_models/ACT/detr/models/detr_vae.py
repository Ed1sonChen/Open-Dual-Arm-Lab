...
class DETRVAE(nn.Module):
    """This is the DETR-VAE module for action chunk prediction."""

    def __init__(self, backbones, transformer, encoder, state_dim, action_dim, num_queries, camera_names):
        """
        Parameters:
            backbones: list of backbone modules
            transformer: transformer decoder module
            encoder: VAE encoder
            state_dim: robot state dimension
            action_dim: action dimension
            num_queries: chunk size
        ...
        self.latent_proj = nn.Linear(hidden_dim, self.latent_dim * 2)  # project hidden state to latent mean/logvar
        ...
        self.additional_pos_embed = nn.Embedding(2, hidden_dim)  # learned embedding for proprio and latent
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None):
        ...
        if is_training:
            ...
            # do not mask cls token and qpos token
            ...
            encoder_output = encoder_output[0]  # CLS token output only
        ...
        if self.backbones is not None:
            ...
            for cam_id, cam_name in enumerate(self.camera_names):
                # NOTE: original repo hardcoded self.backbones[0]
                # keep original behavior to minimize changes
