...
def resolve_task_config(task_name: str):
    is_sim = task_name[:4] == 'sim_'
    if is_sim:
        from constants import SIM_TASK_CONFIGS
        task_config = SIM_TASK_CONFIGS[task_name]
        task_config = dict(task_config)
        task_config.setdefault("state_dim", 7)
        task_config.setdefault("action_dim", 7)
        task_config.setdefault("is_sim", True)
        return task_config, True

    # Method A: prefer local task configs over aloha_scripts.constants.TASK_CONFIGS
    from xarm_task_config import LOCAL_TASK_CONFIGS
    if task_name in LOCAL_TASK_CONFIGS:
        task_config = dict(LOCAL_TASK_CONFIGS[task_name])
        task_config.setdefault("is_sim", False)
        return task_config, False

    # Fallback to official ALOHA task configs if user still wants them
    from aloha_scripts.constants import TASK_CONFIGS
    task_config = dict(TASK_CONFIGS[task_name])
    task_config.setdefault("state_dim", 7)
    task_config.setdefault("action_dim", 7)
    task_config.setdefault("is_sim", False)
    return task_config, False
...
def main(args):
    ...
    # task config
    task_config, is_sim = resolve_task_config(task_name)
...
def eval_bc(config, ckpt_name, save_episode=True):
    ...
    if real_robot:
        # Official repo eval for real robot still assumes ALOHA.
    ...
    for rollout_id in range(num_rollouts):
        ...
        if real_robot:
            # Official ALOHA-only cleanup path.
        ...
        print(f'Rollout {rollout_id}\nepisode_return={episode_return}, highest_reward={episode_highest_reward}, env_max_reward={env_max_reward}, Success: {episode_highest_reward == env_max_reward}')
...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        epoch_summary = compute_dict_mean(
            train_history[len(train_history) - len(train_dataloader):]
        )
